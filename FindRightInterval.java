...
public class FindRightInterval {
    ...
    public int[] input(int[][] intervals) {
        ...
        while (!minheapEnd.isEmpty()) {
            ...
            // Since there are no more elements left in minheapStart, that means rest all
            // indices in result[] will be
            // mapped to -1
            if (minheapStart.isEmpty())
                return result;

            // When minheapStart is not empty, then the top most element of minheapStart
            // must be >= currEndVal
            // So we place the corresponding index of the top most element of minheapStart
